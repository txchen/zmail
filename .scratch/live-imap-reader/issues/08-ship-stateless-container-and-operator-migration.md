...
- [x] README development and Docker instructions describe the Reader shell with Configured Mail accounts, Live IMAP access, browser-memory caching, Manual refresh/retry, and the absence of mail persistence.
...
- [ ] A local container smoke verifies the App login and Reader shell with Configured Mail accounts without requiring Gmail access or a writable data volume.
...
- Added a local `smoke:container` command that builds and inspects the production image, mounts only
  `/config/zmail.toml` read-only, and verifies the App login and Reader shell with Configured Mail
  accounts without opening Gmail.
- Validation passed for typecheck, formatting, lint, 147 non-browser tests, the focused production
  browser smoke, a clean production build, a production server login/Reader-shell smoke, and
