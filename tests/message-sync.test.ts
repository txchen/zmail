...
describe("recent Message sync", () => {
  ...
  it("yields while persisting wide reconciliation results", async () => {
    const persistence = createHybridPersistence();
    const account: ConfiguredMailAccount = {
      id: "personal",
      emailAddress: "me@example.com",
    ...
    const mailDatabase = persistence.mailDatabaseFor("personal");
    mailDatabase.saveMailbox({ id: "inbox", name: "Inbox", unreadCount: 200 });
    ...
    expect(
      mailDatabase.listMessagesForMailbox("personal", "inbox", { limit: 200 }).messages,
    ).toHaveLength(200);
  });
